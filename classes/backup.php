	...
	class pluginbuddy_backupbuddy_backup {
		...
		function backup_zip_files() {
			...
			*/
			...
			//WGA @include_once( $this->_pluginPath . '/lib/wpdbutils/wpdbutils.php' );
			if ( class_exists( 'pluginbuddy_wpdbutils' ) ) {
			//WGA 	// This is the database object we want to use
			 	global $wpdb;
			//WGA 	
			//WGA 	// Get our helper object and let it use us to output status messages
			 	$dbhelper = new pluginbuddy_wpdbutils( $wpdb, $this );
			 	$dbhelper->set_status_callback( array( &$this, 'status' ) );
			//WGA 	
			//WGA 	// If we cannot kick the database into life then signal the error and return false which will stop the backup
			//WGA 	// Otherwise all is ok and we can just fall through and let the function return true
			 	if ( !$dbhelper->kick() ) {
