...
if ( !class_exists( 'PluginBuddyImportBuddy' ) ) {
	class PluginBuddyImportBuddy {
		...
		function PluginBuddyImportBuddy( $parent ) {
			...
			if ( file_exists( dirname( __FILE__ ) . '/backupbb2.php' ) ) {
				echo 'The BB2 importer, Importbb2, can ONLY be accessed on the destination server that you wish to import your backup to.<br>';
			...
			$this->_options = array(
				'zip_id' => '',
				'file' => basename( $_POST[ 'backuppath' ] ),
				'show_php_warnings' => false,
				'type' => 'zip',
				'skip_files' => false,
				'force_compatibility_slow' => false,
			...
			if ( $_POST[ 'backuppath' ] != '' ) {
				...
				if ( $this->_options['zip_id'] == '' ) {
					$this->_options['zip_id'] = $this->get_zip_id( basename( $_POST[ 'backuppath' ] ) );
				}
			}
			
			$wp_uploads = wp_upload_dir();
			
			$this->_options[ 'extract_to' ] = $wp_uploads[ 'basedir' ] . '/bbtmp' .$this->_options[ 'zip_id' ];
		...
		function log( $text, $log_type = 'all' ) {
			...
			$fh = fopen( $this->_options[ 'extract_to' ] . '/importbuddy.txt', 'a');
			if ( $fh !== false ) {
		...
		function view_step_2() {
			...
			echo '<br>Your site will be imported to the following path on the next step. Conflicts may occur if an existing WordPress installation exists here.<br><br><b>Path:</b> ' . $this->_options[ 'extract_to' ];
		...
		function view_step_4() {
			...
			if ( false === $this->_options['skip_files'] ) { // Option to skip all file updating / extracting.
				$this->log( 'STARTING extracting Zip File "' . ABSPATH . '/' . $this->_options['file'] . '" into "' . $this->_options[ 'extract_to' ] . '".' );
				
				echo '<b>Extracting files ' . $this->tip( 'Your site files and/or database must be extracted/unzipped from the ZIP backup file for further processing. This may take some time depending on the size of your size and the speed of the server and server load. If this fails you may need to manually extract the files locally then upload them or use a server tool such as cPanel.', '', false ) . ' into path:</b><br><br>' . $this->_options[ 'extract_to' ] . 'bbtmp' .$this->_options[ 'zip_id' ] . '<br><br>';
			...
			if ( false === $failed ) {
				$this->log( 'SUCCESS extracting Zip File "' . $this->_options[ 'extract_to' ] . '/' . $this->_options['file'] . '" into "' . $this->_options[ 'extract_to' ] . '".' );
				...
				?>
				<h3><?php esc_html_e( 'Step 4 - Migrate Files, Themes, and Plugins', 'it-l10n-backupbuddy' ); ?></h3>
				<form method="post" action="<?php echo esc_url( $form_url ); ?>">
				<?php wp_nonce_field( 'bbms-migration', 'pb_bbms_migrate' ); ?>
				<table class="form-table">
					<tr class="form-field form-required">
						<td>
						<p>Your files have been extracted to a temporary directory.  Proceed to the next step to copy your media, plugins, and themes into the new site.</p>
						</td>
					</tr>
				</table>
				<input type='hidden' name='backuppath' value='<?php echo esc_attr( $_POST[ 'backuppath' ] ); ?>' />
				...
				<input type='hidden' name='upload_url' value='<?php echo esc_attr( $_POST[ 'upload_url' ] ); ?>' />

				<?php submit_button( __('Next Step'), 'primary', 'add-site' ); ?>
				</form>
				<?php
			} else {
				$this->log( 'FAILURE extracting Zip File "' . $this->_options[ 'extract_to' ] . '/' . $this->_options['file'] . '" into "' . $this->_options[ 'extract_to' ] . '".' );
		...
		function view_step_5() {
			...
			$this->log( 'Attempting to copy over media files' );
			$wp_content_dir = $this->_options[ 'extract_to' ] . '/wp-content/uploads';
			$wp_plugins_dir = $this->_options[ 'extract_to' ] . '/wp-content/plugins';
			$wp_themes_dir = $this->_options[ 'extract_to' ] . '/wp-content/themes';
			
			$site_uploads = wp_upload_dir();
			$wp_upload_dir = $site_uploads[ 'basedir' ];
			$wp_upload_url = $site_uploads[ 'baseurl' ];
			$migrate_items = array();
			//Iterate through the temporary directory and get a list of files/directories to copy over
			if ( is_dir( $wp_content_dir ) ) {
				$root_dir = @ opendir($wp_content_dir);
				while (($file = readdir( $root_dir ) ) !== false ) {
					if ( substr( $file, 0, 11 ) != 'backupbuddy' && $file != '.' && $file != '..' ) {
						$migrate_items[] = $wp_content_dir . '/' . $file;
					}
				}
				//echo "</select>";
			} //end is_dir $path
			...
			foreach ( $migrate_items as $filename ) {
				//$this->remove_directory( $wp_upload_dir . '/' . basename( $filename ) );
			}
			//Add stuff
			foreach ( $migrate_items as $filename ) {
			...
			$this->log( 'Successfully copied over the media files' );
			$this->log( 'Attempting to copy over plugin files' );
			?>
			<strong>Media files have been copied over - Now copying over plugins</strong><br />
			<?php
			flush();
			...
			if ( is_dir( $wp_plugins_dir ) ) {
				$migrate_items = array();
				$root_dir = @ opendir( $wp_plugins_dir );
				while (($file = readdir( $root_dir ) ) !== false ) {
					if ( substr( $file, 0, 11 ) != 'backupbuddy' && $file != '.' && $file != '..' ) {
						$migrate_items[] = $wp_plugins_dir . '/' . $file;
					}
				}
				//echo "</select>";
			} //end is_dir $path
			$pluginPath = dirname( $this->_parent->_pluginPath );
			foreach ( $migrate_items as $filename ) {
				if ( file_exists( $pluginPath . '/' . basename( $filename ) ) ) continue;
				$this->_parent->copy( $filename, $pluginPath . '/' . basename( $filename ) );
			}
			?>
			<strong>Plugins files have been copied over - Now copying over themes</strong><br />
			<?php
			flush();
			$this->log( 'Successfully copied over the plugin files' );
			
			$this->log( 'Attempting to copy over theme files' );
			?>
			<?php
			flush();
			...
			if ( is_dir( $wp_themes_dir ) ) {
				$migrate_items = array();
				$root_dir = @ opendir( $wp_themes_dir );
				while (($file = readdir( $root_dir ) ) !== false ) {
					if ( substr( $file, 0, 11 ) != 'backupbuddy' && $file != '.' && $file != '..' ) {
						$migrate_items[] = $wp_themes_dir . '/' . $file;
					}
				}
				//echo "</select>";
			} //end is_dir $path
			...
			foreach ( $migrate_items as $filename ) {
				if ( file_exists( $themePath . '/' . basename( $filename ) ) ) continue;
				$this->_parent->copy( $filename, $themePath . '/' . basename( $filename ) );
			}
			global $current_site;
				$errors = false;	
				$blog = $domain = $path = '';
			...
			?>
			<strong>Theme files have been copied over</strong><br />
			<?php
			flush();
			$this->log( 'Successfully copied over the theme files' );
			?>
			<h3><?php esc_html_e( 'Step 5 - Migrate the database and users', 'it-l10n-backupbuddy' ); ?></h3>
				<form method="post" action="<?php echo esc_url( $form_url ); ?>">
				<?php wp_nonce_field( 'bbms-migration', 'pb_bbms_migrate' ); ?>
				<table class="form-table">
					<tr class="form-field form-required">
						<td>
						<p>Your files have been copied over.  Now begin extracting the database.</p>
						</td>
					</tr>
				</table>
				<input type='hidden' name='backuppath' value='<?php echo esc_attr( $_POST[ 'backuppath' ] ); ?>' />
		...
		function view_step_6() {
			...
			?>
			<strong>Dropped existing tables - Now importing database content</strong><br />
			<?php
			flush();
			
			$this->_options['db_server'] = DB_HOST;
			$this->_options['db_name'] = DB_NAME;
			$this->_options['db_user'] = DB_USER;
			$this->_options['db_password'] = DB_PASSWORD;
			$this->_options['db_prefix'] = $wpdb->prefix;
			...
			$import_result = $this->restore_database( 0 );
			
			?>
			<strong><br />Content has been copied over.  Now doing some data replacements (URL changing).</strong><br />
			<?php
			...
			if ( is_subdomain_install() ) {
				...
				update_option( 'siteurl', 'http://' . $current_site->domain . $_POST[ 'blog_path' ] );
				update_option( 'home', 'http://' . $current_site->domain . $_POST[ 'blog_path' ] );
			}
			...
			/*$old_wpcontent_dir = get_option( 'siteurl' ) . '/wp-content/uploads/';
			$new_wpcontent_dir = get_option( 'siteurl' ) . '/files/';
			
			//$new_wpcontent_dir = get_option
			$sql_statements = array(
				"update {$wpdb->prefix}posts set post_content = REPLACE( {$wpdb->prefix}_posts.post_content, '{$old_wpcontent_dir}', '{$new_wpcontent_dir}' )",
				"update {$wpdb->prefix}posts set guid = REPLACE( {$wpdb->prefix}_posts.guid, '{$old_wpcontent_dir}', '{$new_wpcontent_dir}' )",
				"update {$wpdb->prefix}comments set comment_content = REPLACE( {$wpdb->prefix}_comments.comment_content, '{$old_wpcontent_dir}', '{$new_wpcontent_dir}' )"
			);
			?>*/
			?>
			<strong><br />Content has been migrated.  Proceed to the next step to migrate users.</strong><br />
			<?php
			global $current_site;
				$errors = false;	
				$blog = $domain = $path = '';
			...
			$this->log( 'Successfully copied over the theme files' );
			?>
			<h3><?php esc_html_e( 'Step 6 - Migrate Users', 'it-l10n-backupbuddy' ); ?></h3>
				<form method="post" action="<?php echo esc_url( $form_url ); ?>">
				<?php wp_nonce_field( 'bbms-migration', 'pb_bbms_migrate' ); ?>
				<table class="form-table">
					<tr class="form-field form-required">
						<td>
						<p>Your database has been copied over and migrated.  Now let's import the users.</p>
						</td>
					</tr>
				</table>
				<input type='hidden' name='backuppath' value='<?php echo esc_attr( $_POST[ 'backuppath' ] ); ?>' />
		...
		function view_step_7() {
			...
			$blog_id = isset( $_POST[ 'blog_id' ] ) ? absint( $_POST[ 'blog_id' ] ) : 0;
			switch_to_blog( $blog_id );
			delete_option( $this->_parent->_var ); //Delete BUB options for imported site
			$this->_parent->_parent->delete_cron_hooks( array( 
				$this->_parent->_var . '-cron_process_backup',
				'pb_backupbuddy-cron_scheduled_backup',
			...
			) ); //Get rid of all crons for the site
			//Get all the users
			...
			foreach ( $users as $user ) {
				...
				$sql = "select ID from {$wpdb->users} where user_login = '{$user->user_login}' or user_email = '{$user->user_email}'";
				$user_id = $wpdb->get_var( $wpdb->prepare( $sql ) );
				if ( !$user_id ) {
					//User doesn't exist, let's create it
					$user_args = array();
					foreach ( $user as $key => $user_param ) {
						$user_args[ $key ] = $user_param;
					}
			...
			<?php
			flush();
			$this->log( 'Successfully migrated the users' );
			?>
			<h3><?php esc_html_e( 'Step 7 - Finish Migrating Users and Cleanup', 'it-l10n-backupbuddy' ); ?></h3>
			<form method="post" action="<?php echo esc_url( $form_url ); ?>">
			<?php wp_nonce_field( 'bbms-migration', 'pb_bbms_migrate' ); ?>
			<table class="form-table">
				<tr class="form-field form-required">
					<td>
					<p>Your users have been migrated.  Now it's time for some cleanup.</p>
					</td>
				</tr>
			</table>
			<input type='hidden' name='backuppath' value='<?php echo esc_attr( $_POST[ 'backuppath' ] ); ?>' />
		...
		function view_step_8() {
			...
			$this->log( 'Performing some cleanup' );
			$this->remove_file( ABSPATH . $this->_options[ 'file' ], 'backup .ZIP file (' . $this->_options['file'] . ')', true );
			
			$this->remove_directory( $this->_options[ 'extract_to' ] );
			
			$this->alert( sprintf( "Your new site is now active.  <a href='%s'>Please view it.</a>", $url ) );
			$this->alert( 'Thank you for choosing BackupBuddy!' );			
			$this->log( 'Finished step 8 cleanup! Kerfuffle!' );
		} //end view_step_8
		
		function view_step_9() {
			// Set up backup data from the backupbuddy_dat.php.
			$this->load_backup_dat();
